...
static size_t calcSizeFromDims(const size_t* dims, size_t rank,
                               size_t elementSize) {
  if (rank == 0) return 0;
...
/**
 * @Description: 基于userBUFFER的方法实例化SNPE instance
 */
static void createUserBuffer(
  ...
  Snpe_TensorShape_Handle_t stridesHandle = Snpe_TensorShape_CreateDimsSize(
      strides.data(), Snpe_TensorShape_Rank(bufferShapeHandle));
  size_t bufSize = calcSizeFromDims(
      Snpe_TensorShape_GetDimensions(bufferShapeHandle),
      Snpe_TensorShape_Rank(bufferShapeHandle), bufferElementSize);
  LOG_INFO("Create [{}] buffer size: {}.", name, bufSize);
  // set the buffer encoding type
  Snpe_UserBufferEncoding_Handle_t userBufferEncodingFloatHandle =
      Snpe_UserBufferEncodingFloat_Create();
  // create user-backed storage to load input data onto it
  applicationBuffers.emplace(name, std::vector<uint8_t>(bufSize));
  // create SNPE user buffer from the user-backed buffer
  snpeUserBackedBuffersHandle.push_back(
      Snpe_Util_CreateUserBuffer(applicationBuffers.at(name).data(), bufSize,
                                 stridesHandle, userBufferEncodingFloatHandle));
  // add the user-backed buffer to the inputMap, which is later on fed to the
  // network for execution
  Snpe_UserBufferMap_Add(userBufferMapHandle, name,
                         snpeUserBackedBuffersHandle.back());
...
bool SNPETask::init(const std::string& model_path, const runtime_t runtime) {
  ...
  if (m_container == nullptr) {
    LOG_ERROR("snpe dlc  failed: {}", Snpe_ErrorCode_GetLastErrorString());
  }

  Snpe_SNPEBuilder_Handle_t snpeBuilderHandle =
      Snpe_SNPEBuilder_Create(m_container);
  ...
  std::cout << "m_outputLayers" << m_outputLayers << std::endl;
  std::cout << "snpeBuilderHandle" << snpeBuilderHandle << std::endl;
  std::cout << "m_container" << m_container << std::endl;

  if (Snpe_SNPEBuilder_SetOutputLayers(snpeBuilderHandle, m_outputLayers)) {
    LOG_ERROR("Snpe_SNPEBuilder_SetOutputLayers failed: {}",
  ...
  Snpe_StringList_Handle_t inputNamesHandle =
      Snpe_SNPE_GetInputTensorNames(m_snpe);
  ...
  for (size_t i = 0; i < Snpe_StringList_Size(inputNamesHandle); ++i) {
    ...
    auto bufferAttributesOptHandle =
        Snpe_SNPE_GetInputOutputBufferAttributes(m_snpe, name);
    if (nullptr == bufferAttributesOptHandle) {
      LOG_ERROR("Error obtaining attributes for input tensor: {}", name);
      return false;
    }

    auto bufferShapeHandle =
        Snpe_IBufferAttributes_GetDims(bufferAttributesOptHandle);
    ...
    createUserBuffer(m_inputUserBufferMap, m_inputTensors, m_inputUserBuffers,
                     bufferShapeHandle, name, sizeof(float));
  ...
  Snpe_StringList_Handle_t outputNamesHandle =
      Snpe_SNPE_GetOutputTensorNames(m_snpe);
  ...
  for (size_t i = 0; i < Snpe_StringList_Size(outputNamesHandle); ++i) {
    ...
    // size_t bufferElementSize =
    // Snpe_IBufferAttributes_GetElementSize(bufferAttributesOptHandle);
    createUserBuffer(m_outputUserBufferMap, m_outputTensors,
                     m_outputUserBuffers, bufferShapeHandle, name,
                     sizeof(float));
...
bool SNPETask::setOutputLayers(std::vector<std::string>& outputLayers) {
  ...
  for (size_t i = 0; i < outputLayers.size(); i++) {
    if (SNPE_SUCCESS !=
        Snpe_StringList_Append(m_outputLayers, outputLayers[i].c_str())) {
      LOG_ERROR("Append output name: {} failed: {}.", outputLayers[i],
                Snpe_ErrorCode_GetLastErrorString());
      return false;
    }
  }
...
std::vector<size_t>& SNPETask::getInputShape(const std::string& name) {
  if (isInit()) {
    ...
    LOG_ERROR(
        "The getInputShape() needs to be called after SNPETask is "
        "initialized!");
    // return {};
  }
}
...
std::vector<size_t>& SNPETask::getOutputShape(const std::string& name) {
  if (isInit()) {
    ...
    LOG_ERROR(
        "The getOutputShape() needs to be called after SNPETask is "
        "initialized!");
    // return {};
  }
}
...
float* SNPETask::getInputTensor(const std::string& name) {
  if (isInit()) {
    ...
    LOG_ERROR(
        "The getInputTensor() needs to be called after SNPETask is "
        "initialized!");
    return nullptr;
  }
}
...
float* SNPETask::getOutputTensor(const std::string& name) {
  if (isInit()) {
    ...
    LOG_ERROR(
        "The getOutputTensor() needs to be called after SNPETask is "
        "initialized!");
    return nullptr;
  }
}
...
bool SNPETask::execute() {
  if (SNPE_SUCCESS != Snpe_SNPE_ExecuteUserBuffers(m_snpe, m_inputUserBufferMap,
                                                   m_outputUserBufferMap)) {
