...
ObjectDetectionImpl::~ObjectDetectionImpl() { DeInitialize(); }

bool ObjectDetectionImpl::Initialize(const ObjectDetectionConfig& config) {
  m_task =
      std::move(std::unique_ptr<snpetask::SNPETask>(new snpetask::SNPETask()));
  ...
  // std::cout << "m_outputLayers size: " << m_outputLayers.size() << std::endl;
  for (size_t i = 0; i < m_outputLayers.size(); i++) {
    std::cout << "Output layer: " << m_outputLayers[i] << std::endl;
  }
...
bool ObjectDetectionImpl::PreProcess(cv::Mat& image) {
  ...
  cv::Mat input(inputHeight, inputWidth, CV_32FC3,
                m_task->getInputTensor(m_inputLayers[0]));
  ...
  if (imgHeight != inputHeight || imgWidth != inputWidth) {
    m_scale =
        std::min(inputHeight / (float)imgHeight, inputWidth / (float)imgWidth);

    // std::cout << "m_sclae:" << m_scale << std::endl;

    int scaledWidth = imgWidth * m_scale;
    int scaledHeight = imgHeight * m_scale;

    m_xOffset = (inputWidth - scaledWidth) >> 1;
    m_yOffset = (inputHeight - scaledHeight) >> 1;

    cv::Mat inputMat(inputHeight, inputWidth, CV_8UC3,
                     cv::Scalar(128, 128, 128));
    cv::Mat roiMat(inputMat,
                   cv::Rect(m_xOffset, m_yOffset, scaledWidth, scaledHeight));
    cv::resize(image, roiMat, cv::Size(scaledWidth, scaledHeight),
               cv::INTER_LINEAR);

    // std::cout << "resized image width " << image.cols << "resized image
    // height"
    //           << image.rows << std::endl;
...
bool ObjectDetectionImpl::Detect(cv::Mat& image,
...
bool ObjectDetectionImpl::PostProcess(std::vector<ObjectData>& results,
        ...
        for (int l = 0; l < 3; l++) {              // 3
          for (int m = 0; m < channel / 3; m++) {  // 85
            ...
            if (m < 2) {
              // 处理前两部分
              *tmpOutput =
                  (value * 2 - 0.5 + (m == 0 ? gridX : gridY)) * strides[i];
            } else if (m < 4) {
              // 处理第三部分
              *tmpOutput = value * value * 4 * anchorGrid[i][anchorIdx++];
            } else {
  ...
  for (int i = 0; i < m_grids; i++) {  // 共计MODEL_OUTPUT_GRIDS个grid要统计
    ...
    // m_labels = 85 ----> [bbox_x, bbox_y, bbox_width, bbox_height, bbox_score,
    // prob0, prob1,..., prob84]
    float boxConfidence = m_output[i * m_labels + 4];
    if (boxConfidence > 0.1) {
  ...
  for (size_t i = 0; i < boxIndexs.size(); i++) {
    ...
    int max_idx = 5;
    for (int j = 6; j < m_labels;
         j++) {  // 处理目标分类概率即m_labels中的[prob0, prob1,..., prob84]
      if (m_output[curIdx * m_labels + j] >
          m_output[curIdx * m_labels + max_idx])
        max_idx = j;
    }
    ...
    if (score > m_confThresh) {
      ...
      rect.bbox.x = std::max(0, static_cast<int>(m_output[curIdx * m_labels] -
                                                 rect.bbox.width / 2)) -
                    m_xOffset;
      rect.bbox.y =
          std::max(0, static_cast<int>(m_output[curIdx * m_labels + 1] -
                                       rect.bbox.height / 2)) -
          m_yOffset;
  ...
  for (size_t i = 0; i < winList.size(); i++) {
    if (winList[i].bbox.width >= m_minBoxBorder ||
        winList[i].bbox.height >= m_minBoxBorder) {
      if (!m_roi.empty()) {
