...
class TsProAndCon {
 ...
 public:
  TsProAndCon(unsigned int maxSize) { this->m_maxSize = maxSize; }
  TsProAndCon() { this->m_maxSize = 25; }

  int queuecount() const { return m_queue.size(); }

  void setMaxSize(unsigned int maxSize) { this->m_maxSize = maxSize; }

  unsigned int getMaxSize() { return this->m_maxSize; }

  unsigned int getCurrentSize() { return m_queue.size(); }
