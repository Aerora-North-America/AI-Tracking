...
std::vector<STrack> BYTETracker::update(
  ...
  if (objects.size() > 0) {
    for (int i = 0; i < objects.size(); i++) {
      vector<float> tlbr_;
      tlbr_.resize(4);
      tlbr_[0] = objects[i].bbox.x;
      tlbr_[1] = objects[i].bbox.y;
      tlbr_[2] = objects[i].bbox.x + objects[i].bbox.width;
      tlbr_[3] = objects[i].bbox.y + objects[i].bbox.height;
...
void BYTETracker::remove_duplicate_stracks(std::vector<STrack> &resa,
  ...
  std::vector<vector<float>> pdist = iou_distance(stracksa, stracksb);
...
double BYTETracker::lapjv(const std::vector<vector<float>> &cost,
