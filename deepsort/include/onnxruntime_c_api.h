...
typedef struct OrtCUDAProviderOptions {
   ...
   * size_t) Defaults to SIZE_MAX. \note If a ::OrtArenaCfg has been applied, it
...
typedef struct OrtMIGraphXProviderOptions {
  ...
  int migraphx_fp16_enable;  // enable MIGraphX FP16 precision. Default 0 =
                             // false, nonzero = true
  int migraphx_int8_enable;  // enable MIGraphX INT8 precision. Default 0 =
                             // false, nonzero = true
...
typedef struct OrtOpenVINOProviderOptions {
   ...
   *
   * Valid settings are one of: "CPU_FP32", "GPU_FP32", "GPU_FP16",
   * "MYRIAD_FP16", "VAD-M_FP16" or "VAD-F_FP32"
   */
...
struct OrtApiBase {
  ...
  const char*(ORT_API_CALL* GetVersionString)(void)
      NO_EXCEPTION;  ///< Returns a null terminated string of the version of the
...
struct OrtApi {
   ...
   * ::OrtLoggingFunction `param` parameter to `logging_function`. \param[in]
   * log_severity_level The log severity level. \param[in] logid The log
   * identifier. \param[out] out Returned newly created OrtEnv. Must be freed
   ...
   * input names \param[in] inputs Array of ::OrtValue%s of the input values
   * \param[in] input_len Number of elements in the input_names and inputs
   * arrays \param[in] output_names Array of null terminated UTF8 encoded
   * strings of the output names \param[in] output_names_len Number of elements
   * in the output_names and outputs array \param[out] outputs Array of
   * ::OrtValue%s that the outputs are stored in. This can also be an array of
   * nullptr values, in this case ::OrtValue objects will be allocated and
   ...
   * Sequential Execution mode is enabled (see OrtApi::SetSessionExecutionMode)
   *
   ...
   *
   * Please see
   * https://www.onnxruntime.ai/docs/resources/graph-optimizations.html for an
   * in-depth explanation \param[in,out] options The session options object
   ...
   * OrtApi::SessionGetInputCount returns (exclusive) \param[out] type_info Must
   ...
   * \param[in] session
   * \param[in] index Must be between 0 (inclusive) and what
   * OrtApi::SessionGetOutputCount returns (exclusive) \param[out] type_info
   ...
   * \param[in] session
   * \param[in] index Must be between 0 (inclusive) and what
   * OrtApi::SessionGetInputCount returns (exclusive) \param[in] allocator
   ...
   * \param[in] session
   * \param[in] index Must be between 0 (inclusive) and what
   * OrtApi::SessionGetOutputCount returns (exclusive) \param[in] allocator
   * \param[out] value Set to a null terminated UTF-8 encoded string allocated
   * using `allocator`. Must be freed using `allocator`.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   ...
   * vs GPU etc). \param[in] p_data Pointer to the data buffer. \param[in]
   * p_data_len The number of bytes in the data buffer. \param[in] shape Pointer
   * to the tensor shape dimensions. \param[in] shape_len The number of tensor
   * shape dimensions. \param[in] type The data type. \param[out] out Returns
   ...
   * terminated. \param[in] s_len Count of strings in s (Must match the size of
   * \p value's tensor shape)
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * }<br> \p s must have a size of 11 bytes<br> \p offsets must have 4
   * elements<br> After the call, these values will be filled in:<br> \p s will
   * contain "Thisisatest"<br> \p offsets will contain { 0, 4, 6, 7 }<br> The
   * length of the last string is just s_len - offsets[last]
   *
   * \param[in] value A tensor of type ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING
   * \param[in] s Buffer to sequentially write all tensor strings to. Each
   * string is NOT null-terminated. \param[in] s_len Number of bytes of buffer
   * pointed to by \p s (Get it from OrtApi::GetStringTensorDataLength)
   ...
   * values. \param[in] dim_count Number of elements in `dim_values`
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * failure \param[out] out Newly created ::OrtTensorTypeAndShapeInfo. Must be
   * freed with OrtApi::ReleaseTensorTypeAndShapeInfo
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * points to \param[out] out Newly created ::OrtValue. Must be freed with
   * OrtApi::ReleaseValue
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * destroyed or the name is changed, do not free. \param[out] len Length in
   * bytes of the string returned in `denotation`
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * OrtApi::ReleaseThreadingOptions \snippet{doc} snippets.dox OrtStatus Return
   * Value
   */
   ...
   * metadata map is empty. \param[out] num_keys Set to the number of elements
   * in the `keys` array
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * using OrtApi::ReleaseAvailableProviders \param[out] provider_length Set to
   * the number of entries in the `out_ptr` array
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * returned by OrtApi::GetStringTensorElementLength. \param[in] index Index of
   * the string in the tensor \param[out] s The string element contents in UTF-8
   * encoding. The string is NOT null-terminated.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * key \param[in] config_value A null terminated string representation of the
   * config value
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * output strings and lengths. \param[out] buffer Returns an array of non-null
   * terminated UTF-8 strings. The number of strings stored is returned in the
   * count parameter. This buffer is allocated using `allocator` and must be
   * freed using it. \param[out] lengths Returns an array of `count` lengths of
   * the strings returned in `buffer` This buffer is allocated using `allocator`
   * and must be freed using it. \param[out] count Number of strings returned.
   * If `binding_ptr` has no bound outputs, zero is returned, no memory
   * allocation is performed and buffer and lengths are set to nullptr.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * outputs. Set to nullptr if there are 0 outputs. \param[out] output_count
   * Set to number of ::OrtValue%s returned
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * an element's location relative to its shape \param[in]
   * location_values_count Number of elements in location_values. Must match the
   * number of elements in the tensor's shape. \param[out] out Set to a pointer
   ...
   * ::OrtLoggingFunction `param` parameter to `logging_function`. \param[in]
   * log_severity_level The log severity level. \param[in] logid The log
   * identifier. \param[in] tp_options \param[out] out Newly created OrtEnv.
   ...
   * This option is not needed if the models used have no denormals. Having no
   * denormals is recommended as this option may hurt model accuracy.
   *
   * \param[in] tp_options
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   ...
   * 0 = kNextPowerOfTwo, 1 = kSameAsRequested \param[in]
   ...
   * returned back \param[out] value Set to a null terminated string allocated
   * using `allocator`.  The caller is responsible for freeing it using
   * `allocator`
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * stored \param[in, out] size actual size of attribute array
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * https://onnxruntime.ai/docs/reference/api/c-api.html for details on what
   * the following parameters mean and how to choose these values.): "max_mem":
   * Maximum memory that can be allocated by the arena based allocator. Use 0
   * for ORT to pick the best value. Default is 0. "arena_extend_strategy": 0 =
   * kNextPowerOfTwo, 1 = kSameAsRequested. Use -1 to allow ORT to choose the
   * default. "initial_chunk_size_bytes": (Possible) Size of the first
   * allocation in the arena. Only relevant if arena strategy is
   * `kNextPowerOfTwo`. Use -1 to allow ORT to choose the default. Ultimately,
   * the first allocation size is determined by the allocation memory request.
   * "max_dead_bytes_per_chunk": Threshold of unused memory in an allocated
   * chunk of arena memory after crossing which the current chunk is chunked
   * into 2. "initial_growth_chunk_size_bytes": (Possible) Size of the second
   * allocation in the arena. Only relevant if arena strategy is
   * `kNextPowerOfTwo`. Use -1 to allow ORT to choose the default. Ultimately,
   * the allocation size is determined by the allocation memory request. Further
   ...
   * `arena_config_values` \param[out] out Newly created ::OrtArenaCfg. Must be
   * freed with OrtApi::ReleaseArenaCfg
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * char otherwise) \param[in] options \param[in] prepacked_weights_container
   ...
   * provider options keys \param[in] provider_options_values Array of UTF-8
   * null-terminated string for provider options values \param[in] num_keys
   * Number of elements in the `provider_option_keys` and
   ...
   * strings and lengths. \param ptr - is a UTF-8 null terminated string
   * allocated using 'allocator'. The caller is responsible for using the same
   * allocator to free it.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * custom allocator. See https://onnxruntime.ai/docs/reference/api/c-api.html
   * for details.
   *
   * \param[in] env
   ...
   * output value if any was allocated. \param[in,out] allocator allocator to
   * use when performing an allocation. Allocation will be performed by
   * FillSparseTensor<Format>() APIs. The lifespan of the allocator instance
   * must eclipse the lifespan this sparse tensor instance as the same allocator
   * will be used to free memory. \param[in] dense_shape shape of the original
   * dense tensor \param[in] dense_shape_len number of shape dimensions being
   * passed \param[in] type must be one of TENSOR_ELEMENT_DATA_TYPE_xxxx
   ...
   * copied into a CPU allocated buffer. \param[in] values_shape pointer to
   * values shape array \param[in] values_shape_len length of the values_shape
   * \param[in] values pointer to an array of values. For strings, pass const
   * char**. \param[in] indices_data pointer to a location of COO indices
   ...
   * values shape array \param[in] values_shape_len length of the values_shape
   * \param[in] values - pointer to an array of values. For strings, pass const
   * char**. \param[in] inner_indices_data pointer to a location of CSR inner
   * indices \param[in] inner_indices_num number of CSR inner indices \param[in]
   * outer_indices_data pointer to a location of CSR outer indices \param[in]
   ...
   * that is not the same as mem_info argument to this function a X-device copy
   * will be performed. String data is assumed to be on CPU and will only be
   * copied into a CPU allocated buffer. \param[in] values_shape \param[in]
   * values_shape_len \param[in] values structure with values information
   ...
   * tensor with no non-zero values, pass {0} shape. \param[in] values_shape_len
   * number of values shape dimensions \param[in] type must be one of
   * TENSOR_ELEMENT_DATA_TYPE_xxxx \param[out] out Should be freed by calling
   * ReleaseValue
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * OrtApi::CreateSparseTensorWithValuesAsOrtValue \param[in,out] indices_data
   * pointer to a user pre-allocated buffer or nullptr for fully sparse tensors.
   ...
   *
   * \param[in,out] ort_value ::OrtValue instance constructed with
   * OrtApi::CreateSparseTensorWithValuesAsOrtValue \param[in,out] inner_data
   * pointer to a user pre-allocated buffer or nullptr for fully sparse tensors.
   ...
   * OrtApi::CreateSparseTensorWithValuesAsOrtValue. \param[in,out] outer_data
   * pointer to user pre-allocated buffer or nullptr for fully sparse tensors.
   ...
   *
   * \param[in,out] ort_value OrtValue instance constructed with
   * OrtApi::CreateSparseTensorWithValuesAsOrtValue \param[in] indices_shape
   * pointer to indices shape. Use {0} for fully sparse tensors \param[in]
   * indices_shape_len length of the indices shape \param[in,out] indices_data
   * pointer to user pre-allocated buffer or nullptr for fully sparse tensors.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * tensor \param[out] out Must be freed by
   ...
   * request a format that the sparse tensor does not contain. \param[out] out
   * an instance of ::OrtTensorTypeAndShapeInfo. Must be freed by
   * OrtApi::ReleaseTensorTypeAndShapeInfo
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * \param[in] ort_value ::OrtValue containing sparse tensor.
   * \param[in] indices_format One of the indices formats. It is an error to
   * request a format that the sparse tensor does not contain. \param[out]
   * num_indices Pointer to where the number of indices entries is returned
   ...
   * launch the custom a GPU kernel \see ::OrtCustomOp \param[in]  context
   * OrtKernelContext instance \param[out] out Returns pointer to a GPU compute
   * stream that can be used to launch the custom GPU kernel. If retrieving the
   * GPU compute stream is not relevant (GPU not enabled in the build, kernel
   * partitioned to some other EP), then a nullptr is returned as the output
   * param. Do not free or mutate the returned pointer as it refers to internal
   * data owned by the underlying session. Only use it for custom kernel
   * launching.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * specific API extensions to the OrtApi \param[in] provider_name The name of
   * the execution provider name. Currently only the following values are
   * supported: "DML". \param[in] version Must be ::ORT_API_VERSION. \param[out]
   * provider_api A void pointer containing a reference to the execution
   * provider versioned api structure. For example, the provider_api pointer can
   * be cast to the OrtDmlApi* when the provider_name is "DML".
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * and lengths. \param ptr - is a UTF-8 null terminated string allocated using
   ...
   * of the initializers names. \param[in] initializers Array of ::OrtValue type
   ...
   * or "T1" \param[in] type_constraint_values Type of each contraints
   ...
   */
  ORT_API2_STATUS(
      CreateOp, _In_ const OrtKernelInfo* info, _In_ const char* op_name,
   ...
   * options \param[in] num_keys - number of keys passed in
   *
   ...
   * zdl::DlSystem::Runtime_t::AIP_FIXED_TF. SNPE Runtime_t refers to
   * https://developer.qualcomm.com/docs/snpe/group__c__plus__plus__apis.html
   *   "priority": execution priority, options: "low", "normal".
   *   "buffer_type": ITensor or user buffers, options: "ITENSOR", user buffer
   * with different types - "TF8", "TF16", "UINT8", "FLOAT". "ITENSOR" --
   * default, ITensor which is float only. "TF8" -- quantized model required,
   * "FLOAT" -- for both quantized or non-quantized model If SNPE is not
   * available (due to a non Snpe enabled build or its dependencies not being
...
struct OrtCustomOp {
  ...
  // This callback creates the kernel, which is a user defined parameter that is
  // passed to the Kernel* callbacks below.
